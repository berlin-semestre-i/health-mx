...
class NewSomatometry extends PureComponent {
  ...
  componentDidMount = () => {
    const {pulse, weight, height, temperature, imc } = this.props.somatometry
    this.setState({
      pulse, weight, height, temperature, imc,
    })
  }
  ...
  updateIMC = (name, value) => {
    let imc = 0
    if(name === 'weight')
      imc = getIMC(value, this.state.height/100)
    else
  ...
  save = () => {
    ...
    let enabled = pulse>0 && weight>0 && height>0
    && temperature>0

    if(enabled) {
      this.props.save({ pulse, weight, height, temperature, imc })
    } else {
